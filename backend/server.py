...
class AmbassadorJourneyApplication(BaseModel):
    # Basic info
    name: str  # Journey name/destination
    ...
    # Calendar
    start_date_approximate: str  # e.g., "Março 2026"
    end_date_approximate: Optional[str] = None
    flexibility: str = "flexível"  # flexível | fixo | semi-flexível
    ...
    payment_methods: List[str]  # stripe, mbway, paypal, revolut, wise, crypto
    
    # Optional fields
    poetic_name: Optional[str] = None
    image_url: Optional[str] = None

@api_router.post("/ambassador/apply-journey")
async def submit_ambassador_journey_application(application: AmbassadorJourneyApplication, request: Request):
    """Submit a journey application - Ambassadors only"""
    user = await require_auth(request)
    ...
    if not user_data or user_data.get("level") != "embaixador":
        raise HTTPException(
            status_code=403, 
            detail="Apenas Embaixadores podem submeter candidaturas de viagem. Completa os requisitos para te tornares Embaixador."
        )
    
    # Check if user already has a pending or active journey
    existing_journey = await db.journeys.find_one({
        "ambassador_user_id": user.user_id,
        "status": {"$in": ["candidatura", "aprovada", "ativa"]}
    })
    
    if existing_journey:
        raise HTTPException(
            status_code=400,
            detail=f"Já tens uma viagem em estado '{existing_journey.get('status')}'. Só podes ter uma viagem ativa de cada vez."
        )
    
    # Validate payment methods
    valid_payment_methods = ["stripe", "mbway", "paypal", "revolut", "wise", "crypto"]
    invalid_methods = [m for m in application.payment_methods if m not in valid_payment_methods]
    if invalid_methods:
        raise HTTPException(status_code=400, detail=f"Métodos de pagamento inválidos: {invalid_methods}")
    
    if not application.payment_methods:
        raise HTTPException(status_code=400, detail="Seleciona pelo menos um método de pagamento")
    
    # Validate goal amount
    if application.goal_amount < 100:
        raise HTTPException(status_code=400, detail="O objetivo mínimo é 100€")
    
    if application.goal_amount > 50000:
        raise HTTPException(status_code=400, detail="O objetivo máximo é 50.000€")
    
    # Create journey document
    journey_id = f"journey_{uuid.uuid4().hex[:12]}"
    
    # Generate poetic name if not provided
    poetic_name = application.poetic_name or f"A Jornada de {user_data.get('name', 'um Sonhador')}"
    ...
    image_url = application.image_url or random.choice(default_images)
    ...
    journey_doc = {
        ...
        "emotional_message": application.personal_motivation[:300] if application.personal_motivation else "",
        "impact_description": application.why_this_destination,
        "image_url": image_url,
        "goal_amount": application.goal_amount,
        "current_amount": 0.0,
        ...
        # Location
        "region": get_region_from_country(application.destination_country),
        "country": application.destination_country,
        "city": application.destination_city,
        
        # Ambassador info
        "is_ambassador_journey": True,
        "ambassador_user_id": user.user_id,
        "ambassador_name": user_data.get("name") if user_data.get("use_real_name", True) else user_data.get("anonymous_alias", "Sonhador"),
        
        # Application details (stored for admin review)
        "application_details": {
            "cities_to_visit": application.cities_to_visit,
            "duration_days": application.duration_days,
            "main_activities": application.main_activities,
            "start_date_approximate": application.start_date_approximate,
            "end_date_approximate": application.end_date_approximate,
            "flexibility": application.flexibility,
            "personal_motivation": application.personal_motivation,
            "why_this_destination": application.why_this_destination,
    ...
    await db.notifications.insert_one({
        "notification_id": f"notif_{uuid.uuid4().hex[:12]}",
        "type": "new_journey_application",
        "title": "Nova Candidatura de Viagem",
        "message": f"{journey_doc['ambassador_name']} submeteu uma candidatura para '{application.name}' ({application.destination_country})",
        "journey_id": journey_id,
        "for_admin": True,
        "read": False,
        "created_at": datetime.now(timezone.utc).isoformat()
    ...
    admin_html = f"""
    <h1 style="margin: 0 0 16px 0; color: #2D2A26; font-size: 24px;">📋 Nova Candidatura de Viagem</h1>
    
    <div style="background-color: #E6F4F1; border-radius: 12px; padding: 24px; margin-bottom: 24px;">
        <p style="margin: 0 0 8px 0; color: #6B6661; font-size: 12px; text-transform: uppercase;">Embaixador</p>
        <p style="margin: 0 0 16px 0; color: #2D2A26; font-size: 18px; font-weight: bold;">{journey_doc['ambassador_name']}</p>
        
        <p style="margin: 0 0 8px 0; color: #6B6661; font-size: 12px; text-transform: uppercase;">Destino</p>
        <p style="margin: 0 0 16px 0; color: #2D2A26; font-size: 18px; font-weight: bold;">{application.name}</p>
        
        <p style="margin: 0 0 8px 0; color: #6B6661; font-size: 12px; text-transform: uppercase;">País</p>
        <p style="margin: 0 0 16px 0; color: #2D2A26; font-size: 16px;">{application.destination_country}</p>
        
        <p style="margin: 0 0 8px 0; color: #6B6661; font-size: 12px; text-transform: uppercase;">Objetivo</p>
        <p style="margin: 0 0 16px 0; color: #FFBE98; font-size: 24px; font-weight: bold;">{application.goal_amount}€</p>
        
        <p style="margin: 0 0 8px 0; color: #6B6661; font-size: 12px; text-transform: uppercase;">Duração</p>
        <p style="margin: 0; color: #2D2A26; font-size: 16px;">{application.duration_days} dias</p>
    </div>
    
    <div style="background-color: #FAFAF9; border-radius: 12px; padding: 24px; margin-bottom: 24px;">
        <p style="margin: 0 0 8px 0; color: #6B6661; font-size: 12px; text-transform: uppercase;">Motivação</p>
        <p style="margin: 0; color: #6B6661; font-size: 14px; font-style: italic;">"{application.personal_motivation[:500]}..."</p>
    </div>
    ...
    await send_email_resend(
        to_email=ADMIN_EMAIL,
        subject=f"[Candidatura] {application.name} - {journey_doc['ambassador_name']}",
        html_content=get_email_base_template(admin_html, "Nova Candidatura - 4Luis")
    )
    
    # Remove _id before returning
    journey_doc.pop("_id", None)
    
    return {
        "message": "Candidatura submetida com sucesso! Vamos analisá-la em breve.",
        "journey_id": journey_id,
        "status": "candidatura",
        "journey": journey_doc
    }
...
def get_region_from_country(country: str) -> str:
    ...
    europe = ["portugal", "espanha", "frança", "itália", "alemanha", "reino unido", "holanda", 
              "bélgica", "suíça", "áustria", "grécia", "croácia", "polónia", "república checa",
              "hungria", "dinamarca", "suécia", "noruega", "finlândia", "irlanda"]
    asia = ["japão", "china", "tailândia", "vietname", "indonésia", "malásia", "singapura",
            "coreia", "índia", "nepal", "sri lanka", "filipinas", "camboja", "myanmar"]
    africa = ["marrocos", "egito", "áfrica do sul", "quénia", "tanzânia", "namíbia", 
              "madagascar", "senegal", "etiópia", "gana"]
    americas = ["brasil", "argentina", "chile", "peru", "colômbia", "méxico", "costa rica",
                "cuba", "estados unidos", "canadá", "equador", "bolívia", "uruguai"]
    oceania = ["austrália", "nova zelândia", "fiji", "polinésia", "tahiti", "bali"]
    
    for e in europe:
        if e in country_lower:
            return "europa"
    for a in asia:
        if a in country_lower:
            return "asia"
    for a in africa:
        if a in country_lower:
            return "africa"
...
@api_router.get("/ambassador/my-journeys")
async def get_my_ambassador_journeys(request: Request):
    """Get current user's ambassador journeys"""
    user = await require_auth(request)
