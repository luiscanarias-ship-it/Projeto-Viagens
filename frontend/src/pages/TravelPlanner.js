...
const TravelPlanner = () => {
  ...
  const handleShare = async () => {
    if (!plan) return;
    if (navigator.share) { try { await navigator.share({ title: `Viagem: ${plan.destination}`, text: `Plano de viagem para ${plan.destination} (${plan.dates})`, url: window.location.href }); } catch {} }
    else handleCopy();
  };
